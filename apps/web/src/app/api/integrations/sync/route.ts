...
export async function POST(req: NextRequest) {
  ...
  const { type } = (await req.json()) as { type?: "QUICKBOOKS" | "HUBSPOT" | "ALL" };
  ...
  if (type === "QUICKBOOKS") {
    ...
    await Promise.allSettled([syncQuickBooks(), syncHubSpot()]);
  }

  await db.integration.updateMany({
    where: {
      organizationId: orgId,
      type: { in: Object.keys(results).filter((k) => results[k]!.success).map((k) => k.toUpperCase()) as any },
    },
