...
export function CommandPalette() {
  ...
  const commands: CommandItem[] = [
    ...
    { id: "sales", label: "Ventas", subtitle: "CRM, pipeline, deals", icon: <TrendingUp className="h-4 w-4" />, action: () => navigate("/dashboard/sales"), category: "Navegación", keywords: ["crm", "pipeline"] },
    ...
    { id: "integrations", label: "Integraciones", subtitle: "Meta Ads, SAT, Hojas de cálculo", icon: <Plug className="h-4 w-4" />, action: () => navigate("/dashboard/integrations"), category: "Navegación", keywords: ["conectar", "meta", "sat"] },
    ...
    { id: "sync", label: "Sincronizar integraciones", subtitle: "Actualiza datos desde SAT, Meta", icon: <RefreshCw className="h-4 w-4" />, action: () => { fetch("/api/integrations/sync", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "ALL" }) }); setOpen(false); }, category: "Acciones" },
    {
      id: "theme",
