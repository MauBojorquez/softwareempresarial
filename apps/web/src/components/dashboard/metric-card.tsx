...
export function MetricCard({ title, value, change, icon: Icon, format, trend }: MetricCardProps) {
  ...
  return (
    <div role="article" aria-label={`${title}: ${formattedValue}`} className="group rounded-xl border border-border bg-card p-4 transition-all hover:shadow-md sm:p-5 card-hover animate-scale-in">
      <div className="flex items-center justify-between">
        <p className="text-xs font-medium text-muted-foreground sm:text-sm">{title}</p>
        <div className="rounded-lg bg-primary/8 p-1.5 sm:p-2 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
      ...
      <div className="mt-2 sm:mt-3">
        ...
        {change !== undefined && (
          <p
            className={cn(
              "animate-scale-in mt-1 text-[11px] font-medium sm:mt-1.5 sm:text-xs",
